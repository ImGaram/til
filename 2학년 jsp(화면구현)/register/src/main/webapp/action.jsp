...
try {
	...
	switch(mode) {
	case "insert": sql="INSERT INTO course_tbl VALUES(?,?,?,?,?,?,?)";
		...
		<%
		break;
	case "modify":sql="UPDATE course_tbl SET name=?, credit=?, lecturer=?, week=?, start_hour=?, end_hour=? WHERE id=?";
		pstmt = conn.prepareStatement(sql);
		...
		<%
		break;
	case "delete":sql="DELETE FROM course_tbl WHERE id=?";
		pstmt = conn.prepareStatement(sql);
		pstmt.setString(1, id);
