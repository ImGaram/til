...
try {
	...
	switch(mode) {
	case "insert": sql="INSERT INTO certificate_tbl VALUES(?,?,?,?,?,?,?)";
		...
		<%
		break;
	case "modify":sql="UPDATE certificate_tbl SET name=?, average=?, lecturerid=?, week=?, start_time=?, end_time=? WHERE id=?";
		pstmt = conn.prepareStatement(sql);
		...
		<%
		break;
	case "delete":sql="DELETE FROM certificate_tbl WHERE id=?";
		pstmt = conn.prepareStatement(sql);
		pstmt.setString(1, id);
